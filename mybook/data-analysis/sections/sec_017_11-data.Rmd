...
횡단면 데이터는 **여러 개체**(예: 개인, 기업, 지역 또는 국가)를 **한 시점** 또는 매우 짧은 기간에 관측한 자료다. 이때 **시간은 변동의 주요 차원**이 아니다.

-   각 관측값은 시간에 따라 추적한 동일 개체가 아니라 **서로 다른 개체**를 나타낸다.
...
시계열 데이터는 하나의 개체(또는 집계된 개체)에 대해 여러 기간에 걸쳐 기록한 *동일 변수*의 관측값으로 구성된다. 데이터는 일반적으로 일정한 간격(시간별, 일별, 월별, 분기별 또는 연별)으로 수집되므로 추세, 패턴, 예측을 분석할 수 있다.

예시

-   주식시장: 5년간 한 기업 주식의 일별 종가.
-   경제학: 10년간 한 국가의 월별 실업률.
-   거시경제학: 1960년부터 2020년까지 한 국가의 연도별 GDP.

주요 특성

-   주요 목표는 추세, 계절성, 순환 패턴을 분석하고 미래 값을 예측하는 것이다.
-   시계열 데이터에는 다음과 같은 전문 통계 방법이 필요하다.
...
-   추세, 계절 변동, 경제 주기와 같은 시간적 패턴을 포착한다.
-   경제 지표를 바탕으로 금리를 결정하는 등 예측과 정책 수립에 필수적이다.

과제

-   자기상관: 시간상 가까운 관측값은 서로 상관되는 경우가 많다.
-   구조적 단절: 정책 변화나 경제 위기로 인한 급격한 변화가 분석을 왜곡할 수 있다.
-   계절성: 오해를 낳는 결론을 피하려면 계절성을 고려해야 한다.

------------------------------------------------------------------------

시계열은 일반적으로 네 가지 핵심 요소로 구성된다.

1.  추세(Trend): 시간에 따른 데이터의 장기적 방향 움직임.
2.  계절성(Seasonality): 규칙적이고 주기적인 변동(예: 12월 소매 판매 증가).
3.  순환 패턴(Cyclical Patterns): 불규칙하지만 반복되는 장기 경제 주기.
4.  불규칙(무작위) 요소(Irregular Component): 추세, 계절성 또는 주기로 설명되지 않는 예측 불가능한 변동.
...
### 고정효과와 확률효과의 연결: Mundlak 장치와 Hausman 검정 {#sec-mundlak-hausman}
...
```{r fig-centered-fe-plot, fig.cap="Centered FE Plot", fig.alt="Scatter plot showing centered fixed effects on the x-axis and values from -10 to 20 on the y-axis. Points labeled 5 to 9 are distributed across the chart", out.width="100%", fig.align="center"}
# Plot fixed effects
plot(fixedEffects)
```
...
이는 소표본을 사용할 때 발생하는 **편향**을 보정한다.

------------------------------------------------------------------------

## 올바른 데이터 유형 선택

적절한 데이터 유형의 선택은 다음 사항에 달려 있다.

1.  연구 질문: 개인 수준의 시간에 따른 변화(패널)를 이해해야 하는가, 아니면 한 시점의 스냅숏 비교(횡단면)만 필요한가?

2.  자원: 종단 연구나 패널 연구에는 많은 자원이 필요할 수 있다.

3.  시간 제약: 빠른 결과가 필요하다면 횡단면 또는 반복 횡단면이 더 실용적일 수 있다.

4.  분석 목표: 시계열 예측, 인과 추론, 기술적 비교는 각각 서로 다른 데이터 요구사항을 가진다.

5.  이용 가능성: 때로는 2차 데이터나 반복 횡단면 데이터만 이용할 수 있어 연구 설계가 제한된다.

------------------------------------------------------------------------

## 데이터 품질과 윤리적 고려사항

데이터 유형과 관계없이 데이터 품질은 중요하다. 불완전하거나 편향되었거나 부적절하게 측정된 데이터는 잘못된 결론으로 이어질 수 있다. 연구자는 다음 사항을 지켜야 한다.

-   타당성과 신뢰성 확보: 잘 설계된 도구와 일관된 측정 기법을 사용한다.

-   결측 데이터 처리: 가능하다면 적절한 대치 방법을 적용한다.

-   **탈락 관리**(패널 데이터): 탈락에 대응하기 위해 가중치나 민감도 분석을 고려한다.

-   대표성 확인: 특히 횡단면 및 반복 횡단면 설문에서는 표본추출틀이 목표 모집단과 일치하는지 확인한다.

-   기밀성과 개인정보 보호: 반복적으로 접촉하는 패널 연구에서는 특히 데이터를 안전하게 보관하고 윤리 지침을 따른다.

-   적절한 동의 획득: 참가자에게 연구 세부사항, 데이터 사용 방법, 철회 권리를 알린다.

## 표본 구성 문서화 {#sec-documenting-sample-construction}

모든 실증 논문에는 "120만 개의 기록에서 시작해 가격이 결측인 관측값을 제거하고 기업 특성을 결합한 뒤 21만 4,000개의 분석 표본을 얻었다"와 같은 문장이 들어간다. 이 문장은 긴 조인과 필터의 연쇄를 압축한 것이며, 연쇄의 각 연결부에서 표본이 중요한 방식으로 조용히 바뀔 수 있다. 다대다 병합은 행을 복제해 표본을 부풀릴 수 있고, 내부 조인은 누락될 경우 추정치에 편향을 일으키는 단위를 제거할 수 있다. 데이터의 한 웨이브에 맞춰 작성한 필터가 다음 웨이브에서는 다르게 작동할 수도 있다. 심사자, 재현 연구자, 그리고 소송 지원 업무의 상대방 전문가는 분석 표본이 정확히 어떻게 구성되었는지 물을 것이며, "스크립트를 다시 실행해 보면 된다"는 답변은 받아들여지지 않는다.

이를 해결하는 방법은 감사 추적(audit trail)이다. 즉 각 조인과 필터에 대해 들어온 행 수, 나간 행 수, 그 이유를 기록하는 표를 만든다. 이를 수작업으로 만들면 전사 오류가 발생하기 쉽지만, 일반적인 `dplyr` 동사를 감싸는 작은 래퍼로 자동 생성할 수 있다. 아래 유틸리티는 데이터 프레임을 `TrackedData` 객체로 감싸며, 이 객체의 조인 및 필터 메서드는 모든 단계를 기록한다.

```{r sample-construction-tracker, warning=FALSE, message=FALSE}
library(dplyr)
library(tibble)
library(ggplot2)
...
`dplyr`의 동사는 S3 제네릭이므로 래핑된 객체를 일반적인 파이프라인에 그대로 사용할 수 있다. 아래 예시는 설문을 구매 기록 및 인구통계 자료와 병합한 뒤 분석 필터를 적용한다.
...
이 로그는 그 자체로 출판 가능한 보충 표다. 워터폴 차트는 동일한 정보를 한눈에 읽기 쉽게 보여 주므로 발표와 비기술 독자를 위한 보고서에 유용하다.

```{r sample-construction-waterfall, fig.cap="분석 표본 구성의 워터폴 차트. 각 막대는 조인 또는 필터 단계에 들어오고 나가는 행 수를 나타낸다.", fig.alt="각 조인 및 필터 단계 이후 분석 표본의 행 수를 보여 주는 워터폴 차트. 감소분은 하향 막대로 표시된다."}
...
감사 추적을 가장 유용하게 만드는 습관은 세 가지다. 첫째, 각 필터의 이유를 사후가 아니라 코드를 작성하는 순간 글로 기록한다. 설명 열이 있어야 감사 추적과 단순한 행 수를 구분할 수 있다. 둘째, 예상과 다른 부호가 나타나는 단계는 문제가 없다고 입증될 때까지 버그로 취급한다. 좌측 조인은 행 수를 바꾸면 안 되므로, 이 단계에서 양의 변화가 나타났다면 오른쪽 표의 키가 중복되었다는 신호다. 셋째, 원시 데이터를 새로 고쳤을 때 두 로그를 행 단위로 비교할 수 있도록 분석 결과와 함께 로그를 보관한다. 동일한 표는 점점 더 많은 학술지가 요구하는 탈락 문서이자 소송에서 전문가 보고서가 통상 제공해야 하는 자료로도 활용된다.
