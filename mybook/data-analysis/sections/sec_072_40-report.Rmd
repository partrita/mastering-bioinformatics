...
Figure \@ref(fig:report-modelsummary-demo) shows a quick `modelsummary` regression table and coefficient plot for an `mtcars` OLS fit under three SE specifications.

```{r report-modelsummary-demo, fig.cap="Coefficient plot from `modelplot()` for the `mtcars` OLS fit (`mpg ~ wt + hp + cyl`), comparing classical, robust, and HC4 standard errors; the accompanying `msummary()` regression table is printed inline above the plot.", message=FALSE}
...
The chunk below gives the LaTeX-ready stargazer template for the same three models, with custom dependent-variable and covariate labels.

```{r report-stargazer-latex-demo, eval = FALSE}
...
The chunk below renders the same two OLS regressions as an ASCII (`type = "text"`) stargazer table with 90% confidence intervals.

```{r report-stargazer-ascii, message=FALSE}
...
The snippet below shows the matching stargazer-formatted correlation matrix for the three core `attitude` variables.

```{r report-stargazer-corr, eval = FALSE}
# Correlation table
correlation.matrix <-
...
Pro Tip: Save figures reproducibly for journals. The snippet below is the canonical `ggsave` call to write the most recent ggplot to disk at journal-appropriate dimensions and resolution.

```{r report-ggsave-template, eval = FALSE}
