...
Figure \@ref(fig:nldid-event-vs-true) overlays the estimated ETWFE event-study with the known data-generating-process effect.

```{r nldid-event-vs-true, fig.cap="ETWFE event-study point estimates and 95% confidence band overlaid on the known true dynamic effect from the data-generating process."}
...
Alternatively, we could also use the `plot` function to produce a quick plot. Figure \@ref(fig:nldid-event-ribbon) shows the same event-study via the package's built-in ribbon plot.

```{r nldid-event-ribbon, fig.cap="Default ribbon plot of the ETWFE event-study from marginaleffects::plot, showing the dynamic ATT estimates with their 95% confidence band."}
plot(
    mod_es,
    type = "ribbon",
    # col  = "",# color
...
```{r nldid-event-modelsummary, eval = FALSE}
