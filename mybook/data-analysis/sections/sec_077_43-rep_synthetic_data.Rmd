...
2.  Use a multivariate copula only for `X1` to `X4`.
...
Figure \@ref(fig:repsynth-orig-vs-synth-hist) overlays per-variable histograms for the four original and synthetic columns to provide a visual sanity check that the marginal shapes match.

```{r repsynth-orig-vs-synth-hist, fig.cap="Per-variable overlaid histograms of the four original (red) and synthetic (blue) variables, used as a visual sanity check on marginal fidelity.", message=FALSE}
par(mfrow = c(2, 2))
vars <- c("X1", "X2", "X3", "X4")
...
You should see that the synthetic dataset's means, SDs, and correlation matrix are **very close** to the manually collected values from `df_original`. The plots below show per-variable histograms and the full pairwise scatter-and-correlation chart for the synthetic data.

```{r repsynth-marginal-and-pairwise, message=FALSE}
